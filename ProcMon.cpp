...
BOOL DLLInfo::DependentDLLDispaly()
{
	char arr[200];
...
ProcessInfo::ProcessInfo()
{
	...
	hProcessSanp = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0);
...
BOOL ProcessInfo::ProcessLog()
{
	...
	if(!Process32First(hProcessSanp,&pe32))
	{
...
void DisplayHelp()
{
	cout<<"Developed by Sohel Khan"<<endl;
	...
	cout<<"log	:Create log of current running process ob C drive"<<endl;
	...
	cout<<"exit	:Terminate My ProcMon"<<endl;
		...
		if(count == 1)
		{
		...
		}else if(count == 2)
			{
				...
				delete ppobj;
				  ...
				  if(_stricmp(command[0],"search")==0)
				{
					ppobj = new ProcessInfo();
